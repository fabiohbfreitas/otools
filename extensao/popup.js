...
const clearNaoConsultadosBtn = document.getElementById(
  "clearNaoConsultadosBtn",
);
...
const LOCATION_DETAILS = {
  ...
  Samambaia: {
    endereco:
      "Centro Clínico Samambaia - QS 122 Conjunto 3 lote 01/06 - Samambaia, Brasília - DF, 72304-523",
    maps: "https://maps.app.goo.gl/EgtvqhUE8sEoVxEd8",
  },
};
...
especialidadeSel.addEventListener("change", () => {
  updateDefaultFileName();
  runAllPipelines();
});
  ...
  chrome.scripting.executeScript(
    {
      ...
      func: () => {
        ...
        const confirmTables = document.querySelectorAll(
          'table[id^="tblConfirmacao"], table[id^="tblConsulta"]',
        );
    ...
    (results) => {
      if (chrome.runtime.lastError) {
        toggleLoading(false);
        showStatus(
          "Falha na injeção: " + chrome.runtime.lastError.message,
          "error",
        );
        return;
      }
      ...
      results.forEach((r) => {
        if (r.result) {
          ...
          if (r.result.metaHTMLs) {
            accumulatedMetaHTMLs = accumulatedMetaHTMLs.concat(
              r.result.metaHTMLs,
            );
          }
          if (r.result.confirmHTMLs) {
            accumulatedConfirmHTMLs = accumulatedConfirmHTMLs.concat(
              r.result.confirmHTMLs,
            );
          }
        }
      });
      ...
      naoConsultadosPending =
        accumulatedConfirmHTMLs.length > 0
          ? parseNaoConsultadosHTML(accumulatedConfirmHTMLs)
          : [];
      updateNaoConsultadosUI();
      if (naoConsultadosPending.length > 0) {
        showStatus(
          `${naoConsultadosPending.length} não consultado(s) na página`,
          "success",
          statusNaoConsultadosMsg,
        );
      }
...
function copyRawHtmlToClipboard() {
  ...
  navigator.clipboard
    .writeText(rawTableHTML)
    .then(() =>
      showStatus("HTML copiado para área de transferência!", "success"),
    )
  ...
  metaHtmlArray.forEach((htmlStr) => {
    ...
    rows.forEach((row) => {
      ...
      if (cells.length >= 2) {
        ...
        if (label.includes("unidade executante") && value)
          extractedUnidade = value;
        if ((label.includes("período") || label.includes("periodo")) && value)
          extractedPeriodo = value;
        if (label.includes("procedimento ambulatorial") && value)
          extractedProcedimento = value;
      }
    });
  });
...
function standardiseDataStructure(rawObjectsList) {
  ...
  if (rawObjectsList.length > 0) {
    const firstKeys = Object.keys(rawObjectsList[0]).map((k) =>
      k.toLowerCase(),
    );
    const missing = expectedKeys.filter((key) => {
      if (key === "telefone")
        return !firstKeys.some((k) => k.includes("telefone"));
  ...
  return rawObjectsList.map((obj) => {
    const keyNome =
      Object.keys(obj).find((k) => k.toLowerCase() === "nome") || "";
    const keyTelefones =
      Object.keys(obj).find((k) => k.toLowerCase().includes("telefone")) || "";
    const keyProcedimento =
      Object.keys(obj).find((k) => k.toLowerCase() === "procedimento") || "";
    const keyDataHora =
      Object.keys(obj).find((k) => {
        const lk = k.toLowerCase();
        return (
          lk.includes("data/hora") ||
          (lk.includes("hora") && lk.includes("data"))
        );
      }) || "";
    return {
...
function splitEnviosPhones(phoneStr) {
  const parsed = extractPhones(phoneStr);
  let idx = parsed.findIndex((num) => {
    const digits = num.trim().replace(/\D/g, "");
    const main = digits.substring(2);
    return main.length === 9 && main.startsWith("9");
  });
  if (idx === -1 && parsed.length > 0) idx = 0;
  return {
    principal: idx !== -1 ? parsed[idx] : "",
    outros: idx !== -1 ? parsed.filter((_, i) => i !== idx).join(", ") : "",
  };
}

function buildEnviosDatasets() {
  const refDate = refDateInput.value;
  const chosenEspecialidade = especialidadeSel.value;
  const chosenLocal = localSel.value;
  const details = LOCATION_DETAILS[chosenLocal] || {
    endereco: chosenLocal,
    maps: "",
  };
  const formattedDate = formatToBrazilianDate(refDate);

  const pacientes = [
    ["Nome", "Telefone", "", "Data", "Hora", "Especialidade", "Local"],
  ];
  const envios = [
    [
      "Nome",
  ...
  parsedSourceRows.forEach((row) => {
    ...
    if (chosenEspecialidade) tags.push(chosenEspecialidade);
    if (chosenLocal) tags.push(chosenLocal);
    const agenda = formatAgendaTag(row.Hora);
    if (agenda) tags.push(agenda);
...
function downloadAsMultiSheetExcel(sheets, fileName, statusTarget) {
...
function getSituacaoQualifier(tds) {
  for (const td of tds) {
    ...
    const normalized = text
      ...
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    if (normalized.includes("pendente confirmacao"))
...
function parseNaoConsultadosHTML(confirmHTMLs) {
  ...
  confirmHTMLs.forEach((htmlStr) => {
    ...
    const hasChave = tds.some((td) =>
      td.textContent.trim().startsWith("Chave:"),
    );
    ...
    tds.forEach((td) => {
      ...
      if (
        !solicitacao &&
        label === "" &&
        /^\d{6,}$/.test(bold.textContent.trim())
      ) {
    ...
    doc.querySelectorAll("tr").forEach((row) => {
      ...
      if (
        rowTds.length >= 2 &&
        rowTds[0].textContent.trim().includes("Procedimento")
      ) {
    ...
    results.push({
      ...
      Especialidade:
        extractSubespecialidade(procedimento) ||
        deriveEspecialidade(procedimento),
...
function addNaoConsultadosPending() {
  ...
  const existing = new Set(
    naoConsultadosAccum.map((r) => normalizePacienteKey(r.Paciente)),
  );
  let added = 0;
  ...
  updateNaoConsultadosUI();
  showStatus(
    `${added} adicionado(s). Total: ${naoConsultadosAccum.length}`,
    "success",
    statusNaoConsultadosMsg,
  );
}
...
function downloadNaoConsultadosData() {
  ...
  const structured = naoConsultadosAccum.map((row) => {
    const obj = {};
    NAO_CONSULTADO_HEADERS.forEach((h) => {
      obj[h] = row[h];
    });
    return obj;
  });
...
function getFileName() {
  let exportName = exportNameInput.value
    .trim()
